...
const TACourseView: React.FC = () => {
    ...
    const handleCsvEnroll = async () => {
        ...
        setCsvLoading(true);
        try {
            // Backend expects {id,name,email} objects (it can auto-create student accounts).
            const studentPayload = csvEmails.map(email => {
                ...
                return { id: normalizedEmail, name, email: normalizedEmail };
            });
            const result = await enrollStudentsByCSV(courseId, studentPayload);
            setCsvResult(result);
