...
const FacultyCourseView: React.FC = () => {
    ...
    const handleSaveCourseInfo = async () => {
        ...
        const term = editCourseTerm.trim();
        if (!name || !term) {
            setActionError('Course name and term are required');
            return;
        }
        try {
            await updateCourse(courseId, { name, term });
            ...
            setActionError('Failed to update course info. Please try again.');
        }
    };

    const handleDeleteCourse = async () => {
        if (!courseId || !course) return;
        if (deleteCourseInput !== course.name) {
            setActionError('Course name does not match');
            return;
        }
        try {
    ...
    const handleEnroll = async (studentId: string) => {
        ...
        try {
            ...
            alert('Student enrolled. Their role is now "student". They may need to log out and log in again to see the student dashboard.');
        } catch (err) {
    ...
    const handleInviteTA = async (taId: string) => {
        ...
        try {
            ...
            alert('TA invited. Their role is now "TA". They may need to log out and log in again to see the TA dashboard.');
        } catch (err) {
        ...
        <div className="faculty-course-container">
            ...
            <div className="faculty-course-header">
                ...
                <div className="header-actions">
                    {/* Student list popup (name + ID only) */}
                    <button
                        onClick={() => setShowStudentListModal(true)}
                        className="create-btn"
                        style={{ marginRight: '0.5rem', background: 'var(--bg-surface)', border: '1px solid var(--border-color)', color: 'var(--text-primary)' }}
                    >
                        Student List
                    </button>
            ...
            <div className="course-main-content">
                <div className="assignments-section">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                        ...
                        <Button size="sm" onClick={() => navigate('assignments/new')} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <Plus size={16} />
                            Add assignment
                        </Button>
                    </div>
                ...
                {/* Upcoming assignments (deadline alert; red if due in < 1 day) */}
                <div className="students-section">
                    <h2 className="section-title" style={{ marginBottom: '1rem' }}>Upcoming assignments</h2>
                    <div className="students-list glass">
                        {(() => {
                            ...
                            if (upcoming.length === 0) {
                                return <p className="empty-text">No upcoming assignments.</p>;
                            }
                            return upcoming.map(a => {
                                const dueMs = new Date(a.due_date).getTime();
                                const isUrgent = dueMs - now < oneDayMs;
                                return (
                                    <div
                                        key={a.id}
                                        className="student-item"
                                        style={isUrgent ? { background: 'rgba(239, 68, 68, 0.12)', borderLeft: '4px solid #ef4444' } : undefined}
                                        onClick={() => navigate(`assignments/${a.id}/grading`)}
                                        role="button"
                                        tabIndex={0}
                                        onKeyDown={(e) => e.key === 'Enter' && navigate(`assignments/${a.id}/grading`)}
                                    >
                                        <div className="student-info" style={{ flex: 1, cursor: 'pointer' }}>
                                            <Calendar size={18} style={{ color: isUrgent ? '#ef4444' : 'var(--text-secondary)', flexShrink: 0 }} />
                                            <div>
                                                <p className="student-name" style={isUrgent ? { color: '#b91c1c', fontWeight: 600 } : undefined}>{a.title}</p>
                                                <p className="student-email" style={{ margin: 0, fontSize: '0.8rem' }}>
                                                    Due {new Date(a.due_date).toLocaleString(undefined, { dateStyle: 'short', timeStyle: 'short' })}
                                                    {isUrgent && <span style={{ color: '#ef4444', fontWeight: 600, marginLeft: '8px' }}>— Due in less than 1 day</span>}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="archive-modal-card">
                        <div className="archive-header">
                            <div className="archive-title-container">
                                ...
                                <h2 className="archive-title">Edit Course Info</h2>
                            </div>
                            <button className="archive-close-btn" onClick={() => setShowEditCourseModal(false)}>
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="archive-body">
                            {actionError && (
                                <p style={{ color: 'var(--danger-color)', fontSize: '0.85rem', marginBottom: '0.75rem' }}>{actionError}</p>
                            )}
                            <div className="archive-input-container">
                                <label style={{ fontSize: '0.8rem', fontWeight: 500, marginBottom: '0.25rem' }}>Course ID</label>
                                <input type="text" value={course.id} disabled />
                            </div>
                            <div className="archive-input-container">
                                <label style={{ fontSize: '0.8rem', fontWeight: 500, marginBottom: '0.25rem' }}>Course Name</label>
                                <input
                                    type="text"
                            ...
                            <div className="archive-input-container">
                                <label style={{ fontSize: '0.8rem', fontWeight: 500, marginBottom: '0.25rem' }}>Term</label>
                                <input
                                    type="text"
            ...
            {/* Student List Modal (name + ID only) */}
            {showStudentListModal && (
                <div className="archive-modal-overlay" onClick={() => setShowStudentListModal(false)}>
                    <div className="archive-modal-card" onClick={e => e.stopPropagation()}>
                        <div className="archive-header">
                            <div className="archive-title-container">
                                ...
                                <h2 className="archive-title">Enrolled Students</h2>
                            </div>
                            <button className="archive-close-btn" onClick={() => setShowStudentListModal(false)}>
                                <X size={20} />
                            </button>
                        </div>

                        <div className="archive-body" style={{ maxHeight: '320px', overflowY: 'auto' }}>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                                    <thead>
                                        <tr>
                                            ...
                                            <th style={{ textAlign: 'left', padding: '0.4rem 0.25rem' }}>ID</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {enrolledStudents.map(s => (
                                            <tr key={s.id}>
                                                <td style={{ padding: '0.35rem 0.25rem', borderTop: '1px solid var(--border-color)' }}>
                                                    {s.name}
                                                </td>
                                                <td style={{ padding: '0.35rem 0.25rem', borderTop: '1px solid var(--border-color)', fontFamily: 'monospace', fontSize: '0.8rem', color: 'var(--text-tertiary)' }}>
                                                    {s.id}
                                                </td>
                                            </tr>
                                        ))}
            ...
            {showDeleteCourseModal && course && (
                <div className="archive-modal-overlay">
                    <div className="archive-modal-card">
                        <div className="archive-header">
                            <div className="archive-title-container">
                                <div className="archive-icon-container" style={{ background: 'rgba(239, 68, 68, 0.2)' }}>
                                    <Trash2 size={22} style={{ color: '#ef4444' }} />
                                </div>
                                <h2 className="archive-title">Delete Course?</h2>
                            </div>
                            <button className="archive-close-btn" onClick={() => { setShowDeleteCourseModal(false); setActionError(null); }}>
                                <X size={20} />
                            </button>
                        </div>
                        <div className="archive-body">
                            <p className="archive-instruction">
                                This will permanently delete the course, all assignments, enrollments, and submissions. Type <span style={{ fontWeight: 800, color: 'var(--primary-color)' }}>{course.name}</span> to confirm.
                            </p>
                            <div className="archive-input-container">
                                <input
                                    type="text"
                                    ...
                                    onChange={(e) => {
                                        setDeleteCourseInput(e.target.value);
                                        setActionError(null);
                                    }}
                                    placeholder="Type course name"
                                    autoFocus
                            ...
                            <div className="archive-warning">
                                <AlertTriangle size={16} />
                                <p className="archive-warning-text">This action cannot be undone.</p>
                            </div>
                            <div className="archive-footer">
                                <button onClick={() => { setShowDeleteCourseModal(false); setActionError(null); }} className="archive-btn archive-btn-cancel">Cancel</button>
                                <button onClick={handleDeleteCourse} disabled={deleteCourseInput !== course.name} className="archive-btn" style={{ background: '#ef4444', color: 'white' }}>Delete Course</button>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <input
                                ...
                                onChange={async (e) => {
                                    ...
                                    if (e.target.value.trim().length >= 2) {
                                        setIsSearching(true);
                                        try {
                                            const res = await searchTAs(e.target.value.trim());
                                ...
                                placeholder="Search any user by name or email (students & unassigned users can be invited as TA)..."
                        ...
                        <div className="search-results-container custom-scrollbar">
                            {isSearching ? (
                                    ...
                                    <div key={ta.id} className="search-result-item">
                                        <div className="search-result-info">
                                            ...
                                            <div>
                                                ...
                                                <span className="search-result-role" style={{ fontSize: '0.7rem', color: 'var(--text-tertiary)', textTransform: 'capitalize' }}>
                                                    {ta.role == null || ta.role === '' ? 'User' : ta.role}
                                                </span>
                                            </div>
                                        </div>
                                        <button
                                ...
                                taSearchQuery.length > 0
                                    ? <p className="text-center py-4 text-gray-500">No user found matching your search.</p>
                                    : <p className="text-center py-4 text-xs text-gray-400 font-medium">Type to search any user to invite as TA (students and unassigned users will get TA role).</p>
                            )}
                        </div>
